...
/**
 * An index which stores objects matching any type of query, and which can
 * answer arbitrarily complex queries in constant time complexity.
 * <p/>
 * A standing query index must be added ahead of time for the exact query which
 * is to be accelerated.
 * <p/>
 * Note that it is possible to add a standing query index on any <i>fragment</i>
 * of a query, as well as whole queries. CQEngine will accelerate evaluation of
 * query fragments (branches, or nested sub-trees within queries) using any
...
public class StandingQueryIndex<O> implements Index<O>, OnHeapTypeIndex {
	...
	private final StoredResultSet<O> storedResultSet = new StoredSetBasedResultSet<O>(
			Collections.<O>newSetFromMap(new ConcurrentHashMap<O, Boolean>()), INDEX_RETRIEVAL_COST);
	 ...
	 * {@inheritDoc}
	 * <p/>
	 * <b>This implementation always returns true, as this index supports all types
	 * of query.</b>
	 *
	...
	public boolean replacePreviousValueWithNewOne(O prevValue, O newValue, QueryOptions queryOptions) {
      ...
      // TODO : should this be moved deeper and made atomic?
			if (standingQuery.matches(prevValue, queryOptions)) {
				modified |= storedResultSet.remove(prevValue);
			}
			if (standingQuery.matches(newValue, queryOptions)) {
				modified |= storedResultSet.add(newValue);
			}
		}
		return modified;
	}
	...
	public String toString() {
		return "StandingQueryIndex{" + "standingQuery=" + standingQuery + '}';
	}
	 ...
	 * <p/>
	 * Equivalent to
	 * {@code new StandingQueryIndex&lt;Query&lt;O&gt;, O&gt;(standingQuery)}.
	 * <p/>
	 *
	 ...
	 * @return A new StandingQueryIndex which will build an index on this standing
	 *         query
	 */
